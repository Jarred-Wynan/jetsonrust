...
impl App {
    ...
    > {
        ...
        "nvarguscamerasrc sensor-id=0 ! video/x-raw(memory:NVMM),width=3840,height=2160,framerate=30/1 ! nvv4l2h264enc insert-sps-pps=true ! h264parse ! rtph264pay config-interval=-1 pt=96 ! webrtcbin. webrtcbin name=webrtcbin"
...
async fn run(
    ...
    println!("Rust started with version from Wednesday 27th March, insert-sps-pps=true");
...
// #[derive(Debug, StructOpt)]
// struct Args {
//     #[structopt(short, long, default_value = "ws://10.42.0.69:8443")]
//     server: String,
//     #[structopt(short, long)]
//     peer_id: Option<u32>,
// }
...
//         let pipeline = gst::parse_launch(
//             "videotestsrc pattern=ball is-live=true ! vp8enc deadline=1 keyframe-max-dist=2000 ! rtpvp8pay name=vpay pt=96 picture-id-mode=15-bit ! webrtcbin. webrtcbin name=webrtcbin"
//             //"nvarguscamerasrc sensor-id=0 ! video/x-raw(memory:NVMM),width=1920,height=1080,framerate=30/1 ! nvv4l2h264enc insert-sps-pps=true ! h264parse ! rtph264pay config-interval=1 pt=96 ! webrtcbin. webrtcbin name=webrtcbin"
//     )?;
